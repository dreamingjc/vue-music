<template>
  ...
  >
      ...
      <li class="list-group" v-for="(group, index) in data" :key="index" ref="listGroup">
        ...
        <ul>
          <li @click="selectItem(item)" class="list-group-item" v-for="(item, index) in group.items" :key="index">
            <img v-lazy="item.avtar" alt="" class="avatar" />
    ...
    >
      <ul>
        <li class="item" :class="{'current': currentIndex === index}" v-for="(item, index) in shortcut" :key="index" :data-index="index">{{item}}</li>
      </ul>
    </div>
  ...
  computed: {
    shortcut() {
      ...
      this.data.forEach(item => {
        shortcut.push(item.title.substr(0, 1))
      })
      return shortcut
    },
    fixedTitle() {
